...
CONFIG = {
    ...
    "xgb_params": {
        ...
        "learning_rate": 0.03,      # 0.02~0.04 区间；越小越慢
        "max_depth": 0,             # 不限深，用 max_leaves 控制容量
        "max_leaves": 192,          # 127~255 之间；192 是速度/精度折中
        "grow_policy": "lossguide",
        "max_bin": 256,             # 256 明显快于 512；精度常几乎不掉
        "min_child_weight": 2.0,    # 稍增稳泛化，减少无效分裂
        "lambda": 8.0,              # L2 正则（5~12 可扫）
        "alpha": 0.5,               # L1 正则（0~1 可扫）
        "gamma": 0.0,               # 如过拟合可设 0.05~0.2
        "subsample": 0.9,           # 轻采样；再快一点可 0.85
        "colsample_bytree": 0.9,
        "colsample_bylevel": 0.9,
        "colsample_bynode": 0.9,
        "enable_categorical": True, # 直接吃 pandas category
        "max_cat_to_onehot": 128,   # 降低高基数 one-hot 消耗
    ...
    "cache_dir": "data/processed_v2",  # 缓存目录
}
...
def main(cfg: dict):
    ...
    if cfg["save_importance"]:
        fi = pd.concat(out["importances"]).groupby("feature")["gain"].mean().sort_values(ascending=False)
        fi.to_frame("gain").to_csv(os.path.join(cfg["out_dir"], "feature_importance_gain.csv"))
        print("[save] importance ->", os.path.join(cfg["out_dir"], "feature_importance_gain.csv"))
