...
CONFIG.update({
    ...
    # —— 容量与学习率 —— #
    "depth": 8,                    # 比 A 版更深
    "learning_rate": 0.025,        # 稍低 lr 提升稳健
    "l2_leaf_reg": 12.0,           # 更强 L2

    # —— 采样/随机性（与 A 版不同）—— #
    "bootstrap_type": "MVS",       # 与 A 的 Bayesian 不同
    "bagging_temperature": 1.0,    # 提高随机性
    "random_strength": 2.0,

    # —— 连续特征分箱 —— #
    "border_count": 254,           # 更细的分箱（稍慢）

    # —— 叶子估计 —— #
    "leaf_estimation_iterations": 4,

    # —— 训练控制 —— #
    "iterations": 10000,           # 会被早停截断
    "early_stopping_rounds": 600,
